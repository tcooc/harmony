  ...
  start() {
    if (!this.currentlyPlaying) {
      if (this.queue.length) {
        ...
        var dispatcher = this.voiceConnection.playStream(
          playable.stream,
          _.extend({}, this.defaultOptions, playable.options)
        );
        dispatcher.on('start', () => {});
        dispatcher.on('end', () => {
          this.stop();
  ...
  _addFromUrl(urlString, output) {
    ...
    if (url.hostname === 'www.youtube.com' && url.pathname === '/watch') {
      ...
      addPromise = request
        ...
        .then(response => {
          if (response.statusCode !== 200) {
            throw new Error(
              'invalid status from google playlist api ' + response.statusCode
            );
          }
          var playlist = JSON.parse(response.body);
          var toAdd = _(playlist.items)
            .chain()
            .map(item => {
      ...
      addPromise = Promise.resolve(
        new TwitchPlayable(urlString, this.settings.twitch.client_id, output)
      );
    } else {
