...
function youtubeIdToUrl(videoId) {
...
AudioManager.prototype._playYoutube = function(spec) {
	...
	var youtubeUrl = youtubeIdToUrl(videoId);
	...
	if(url.hostname === 'www.youtube.com' && url.pathname === '/watch') {
		...
		return request.getAsync({
			...
			_.each(playlist.items, function(item) {
				var itemUrl = youtubeIdToUrl(item.contentDetails.videoId);
				this.queue.push({type: AudioTypes.Youtube, output: output, url: itemUrl, options: options});
			}.bind(this));
			this.start();
