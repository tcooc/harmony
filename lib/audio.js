...
AudioManager.prototype.stop = function() {
	if(this.client.voiceConnection) {
		this.client.voiceConnection.stop();
...
AudioManager.prototype._playYoutube = function(spec) {
	...
	stream.on('info', function(info, format) {
		...
		logger.debug(info);
		logger.debug(format);
		this.client.sendMessage(spec.output, 'Playing **' + info.title + '**');
	});
	...
	this._setCurrentlyPlaying({
		spec: spec,
		stop: function() {
			this.client.sendMessage(spec.output, 'Finished playing');
			if(stream.end) {
				stream.end();
			}
			if(stream.destroy) {
...
AudioManager.prototype._playTwitch = function(spec) {
	...
	this._setCurrentlyPlaying({
		spec: spec,
		stop: function() {
			this.client.sendMessage(spec.output, 'Stream stopped');
			process.kill('SIGINT');
		}
	});
	this._playStream(spec.output, stream, spec.options);
	this.client.sendMessage(spec.output, 'Playing stream ' + spec.url);
};
...
AudioManager.prototype.play = function(output, urlString, options) {
	...
	if(url.hostname === 'www.youtube.com' && url.pathname === '/watch') {
		...
		this.client.sendMessage(output, 'Your link is broked');
	}
	this.start();
};
