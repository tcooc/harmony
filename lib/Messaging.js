...
class MessageSender {
	...
	popQueue() {
		if(this.queue.length === 0) {
			...
			promise.then(() => this._handleSent(payload)).catch((e) => this._handleError(e, payload))
	...
	send(channel, ...args) {
		return new Promise((resolve, reject) => {
			...
			if(this.queue.length <= this.backlog) {
				this.queue.push({channel: channel, args: args, resolve: resolve, reject: reject});
			} else {
...
class Messaging {
	...
	getPrefix(message) {
		var guild = message.guild, member;
