...
Messaging.prototype.getPrefix = function(message) {
	...
	if(message.channel.server) {
		prefix = (this.settings.db._data.prefix.find((prefix) => prefix.server === message.channel.server.id) || this.settings.defaultPrefix).prefix;
	}
	if(prefix === '@<me>') {
...
Messaging.prototype.setPrefix = function(message, prefix) {
	if(message.channel.server) {
		...
		this.settings.db.update(function(data) {
			var found = data.find((prefix) => prefix.server === message.channel.server.id);
			if(found) {
				data.prefix[data.prefix.indexOf(found)] = newPrefix;
			} else {
				data.prefix.push(newPrefix);
			}
		});
		return true;
	}
