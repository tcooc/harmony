...
Messaging.prototype.getPrefix = function(server) {
	var prefix = (this.settings.prefix.find('server', server.id) || this.settings.defaultPrefix).prefix;
	if(prefix === '@<me>') {
...
Messaging.prototype.process = function(message) {
	logger.debug('Prefix for server ' + message.channel.server.id + ' is ' + this.getPrefix(message.channel.server));
