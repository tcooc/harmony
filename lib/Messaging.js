...
class Messaging {
	...
	isOfficer(member, guild) {
		if(member.id === this.settings.owner) {
			return true;
		}
		if(member instanceof Discord.User) {
			// member is a User object, find connected GuildMember
			member = guild.members.find('id', member.id);
		}
		return member ? member.hasPermission('MANAGE_ROLES_OR_PERMISSIONS') : false;
	}
