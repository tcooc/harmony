  ...
  app.get('/:name/search', async (req, res) => {
    ...
    for (const [uniqueName, item] of Object.entries(warframeLoadoutData)) {
      if (item.name.toLowerCase().indexOf(query) > -1) {
        matches[uniqueName] = item.name;
      }
    }

    const matchedItems = player.items.filter(
      ({ uniqueName }) => matches[uniqueName]
    );

    if (matchedItems.length) {
      // prioritize primes
      const matchedPrimes = matchedItems.filter(
        ({ uniqueName }) =>
          matches[uniqueName].toLowerCase().indexOf('prime') > -1
      );
      const match = matchedPrimes.length ? matchedPrimes[0] : matchedItems[0];
      res.end(`${matches[match.uniqueName]} build: ${match.href}`);
    } else {
