...
module.exports = function(messaging, client) {
  ...
  function doUpdateAlertMessage(message, timestamp, content) {
    ...
    if (expiresIn <= 0) {
      ...
      var newContent = message.content.replace(
        ALERT_TWEET_REGEX,
        '- ' + Math.round(expiresIn) + 'm -'
      );
      var promise =
      ...
      return promise.then(function(message) {
        ...
        setTimeout(
          () => doUpdateAlertMessage(message, timestamp, content),
          10 * SECOND
        );
      });
    }
  ...
  messaging.addCommandHandler(/^!alertme/i, function(message, content) {
    if (!messaging.hasAuthority(message)) {
      messaging.send(
        message.author,
        "You don't seem to have permission to set alerts for that channel.\n" +
    ...
    var index = _.findIndex(
      broadcasts,
      broadcast =>
        broadcast.for === message.channel.id ||
        broadcast.for === message.author.id
    );
    if (watchList.length === 0 || !correctList) {
      var response = ['Subscribe to warframe alerts'];
      if (index > -1) {
        response.push(
          'Your current watch list is: `' +
            broadcasts[index].accept.source.split('|').join(' ') +
            '`'
        );
      }
      response.push(
        'To set an alert, message me a comma separated list of items, like `!alertme Reactor Catalyst Forma Nitain`'
      );
      ...
      messaging.send(
        message,
        'Watching for `' + pattern.split('|').join(' ') + '`'
      );
    }
    return true;
  });
