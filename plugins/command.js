...
const { CustomCommand } = require('../lib');
...
module.exports = function(messaging) {
  ...
  messaging.addCommandHandler(/^!custom:list/i, function(message) {
    ...
    if (id) {
      ...
      messaging.send(
        message,
        'Use this command in a server to print server commands'
      );
    }
    return true;
  });
  ...
  messaging.addCommandHandler(/^!custom:source/i, function(message, args) {
    ...
    if (id) {
      ...
      messaging.send(
        message,
        'Use this command in a server to see command source'
      );
    }
    return true;
  });
  ...
  messaging.addCommandHandler(/.*/, function(message, args) {
    ...
    if (args.length) {
      ...
      if (_.find(ids, id => !commandCache[id])) {
        // if there exists uncached commands
        promise = db
          .get()
          .then(data => _.each(ids, id => refreshCache(id, data)));
      } else {
      ...
      return promise
        .then(function() {
          return _(ids)
            ...
            .find(command => command.process(message))
            .value();
        })
        .catch(function(e) {
