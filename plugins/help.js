...
const commandSpecs = {
  '/^!d3gem/i': [
    '!d3gem <gemlevel>',
    'Other',
    'Diablo 3: Show helpful info on levelling legendary gems'
  ],
  '/^!eval/i': false,
  ...
  '/^!alertme/i': [
    '!alertme <space separated list of alert items>',
    'Warframe',
  ...
  '/^!wiki/i': [
    '!wiki',
    'Warframe',
    'Get wiki link for the specified topic. For example, `!wiki excalibur`'
  ],
  '/^!warframe:news/i': [
    '!warframe:news <lang>',
    'Warframe',
  ...
  '/^!audio:loop/i': [
    '!audio:loop <loop(yes/no)> <shuffle(yes/no)>',
    'Audio',
    'Set looping and shuffling'
  ],
  ...
  '/^!audio:clear/i': [
    '!audio:clear',
    'Audio',
    'Clear playlist and currently playing'
  ],
  '/^!audio:leave/i': ['!audio:leave', 'Audio', 'Bot will leave voice channel'],
  '/^!shodan/i': false,
  '/^!custom:add/i': [
    '!custom:add <command> <message>',
    'Custom Commands',
  ...
  '/^!custom:list/i': [
    '!custom:list',
    'Custom Commands',
    'List custom commands for server'
  ],
  '/^!custom:source/i': [
    '!custom:source <command>',
    'Custom Commands',
    'See command source for command'
  ],
  '/.*/': false,
  ...
  '/^!mcstatus/i': [
    '!mcstatus <server>',
    'Other',
    'Minecraft: Check server status'
  ],
  '/^!price/i': ['!price <item name>', 'Warframe', 'Price check'],
  '/^!twitch/i': [
    '!twitch <twitch username>',
    'Announce when user is streaming'
  ]
};
...
module.exports = function(messaging, client) {
  ...
  messaging.addCommandHandler(/^(!|\/)?(commands?)|(help)|(about)/i, function(
    message
  ) {
  ...
  messaging.addCommandHandler(/^!feedback/i, function(message) {
    messaging.send(
      client.users.get(messaging.settings.owner),
      'Feedback: `' +
        message.content +
        '` from ' +
